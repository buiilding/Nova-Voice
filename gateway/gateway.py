...
if not JWT_SECRET:
    raise ValueError("JWT_SECRET environment variable is required. Please set it in gateway/.env file")
...
class GatewayService:
    def __init__(self):
        ...
        self.auth_middleware = AuthMiddleware(JWT_SECRET)
    ...
    async def create_or_update_user(self, user_data: dict) -> str:
        """Create or update user from OAuth data (supports multiple providers)"""
        # Determine provider and user identifier
        provider = None
        provider_id = None
        ...
        if 'google_id' in user_data:
            ...
            user_id = f"user:{user_data['google_id']}"
        elif 'github_id' in user_data:
            provider = 'github'
            provider_id = user_data['github_id']
            user_id = f"user:github_{user_data['github_id']}"
        elif 'microsoft_id' in user_data:
            provider = 'microsoft'
            provider_id = user_data['microsoft_id']
            user_id = f"user:microsoft_{user_data['microsoft_id']}"
        elif 'discord_id' in user_data:
            provider = 'discord'
            provider_id = user_data['discord_id']
            user_id = f"user:discord_{user_data['discord_id']}"
        else:
            raise ValueError("No valid OAuth provider ID found in user data")
        ...
        if existing_user:
            ...
            existing_data_raw = await self.redis_client.redis.hgetall(user_id)
            ...
            if 'name' in user_data:
                existing_data['name'] = user_data['name']
            if 'avatar' in user_data:
                existing_data['avatar'] = user_data['avatar']
            ...
            await self.redis_client.redis.hset(user_id, mapping=encoded_data)
        else:
            # Create new user
            user_data_redis = {
                f'{provider}_id': provider_id,
            ...
            # Encode strings to bytes for Redis
            encoded_user_data = {}
            for key, value in user_data_redis.items():
                if isinstance(key, str):
                    key = key.encode('utf-8')
                if isinstance(value, str):
                    value = value.encode('utf-8')
                encoded_user_data[key] = value
            await self.redis_client.redis.hset(user_id, mapping=encoded_user_data)
    ...
    async def find_user_by_email(self, email: str) -> dict:
        """Find existing user by email across all providers"""
        try:
            # Scan all user keys to find matching email
            user_keys = []
            async for key in self.redis_client.redis.scan_iter("user:*"):
            ...
            for user_key in user_keys:
                ...
                user_data_raw = await self.redis_client.redis.hgetall(user_key)
                # Decode bytes to strings
                user_data = {}
