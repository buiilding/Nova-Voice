...
try {
  ...
  console.warn('[TYPING] robotjs unavailable. Voice typing output disabled until dependency is fixed.', error.message);
}
...
const GATEWAY_URL = process.env.GATEWAY_URL || 'ws://localhost:5026';
...
function createWindow() {
  ...
  // Linux: standard window for simple voice-typing workflow.
  // Other platforms: keep existing compact overlay config.
  mainWindow = new BrowserWindow({
    width: isLinux ? 580 : 100,
    height: isLinux ? 520 : 100,
    x: (width - (isLinux ? 580 : 100)) / 2,
    y: isLinux ? Math.max(40, Math.floor(height * 0.12)) : 0,
    transparent: !isLinux,
    backgroundColor: isLinux ? '#020617' : '#00000000',
    frame: isLinux,
    alwaysOnTop: !isLinux,
    resizable: isLinux,
    skipTaskbar: !isLinux,
    ...
    webPreferences: {
      ...
      permissions: ['microphone', 'media', 'display-capture']
    },
...
function registerGlobalShortcuts() {
  if (process.platform === 'linux') {
    console.log('[SHORTCUTS] Skipped on Linux simple UI mode');
    return;
  }
...
ipcMain.handle('set-mode', (event, mode) => {
  if (mode === 'typing') {
    if (!robot) {
      return { success: false, error: 'robotjs is unavailable. Rebuild/install robotjs before using voice typing.' };
    }
    ...
    if (process.platform === 'linux') {
      return { success: false, error: 'Subtitle mode is disabled in Linux simple UI mode.' };
    }
