// In production, variables are injected from build-env.json. In development, we use dotenv
...
function getPreloadPath(scriptName) {
  ...
  console.log(`[PRELOAD] app.isPackaged: ${app.isPackaged}`);
  return preloadPath;
}

// Use app.isPackaged to reliably detect production mode vs. development mode
if (app.isPackaged) {
  // When running from a packaged app, load the baked-in config
  try {
    const envConfig = JSON.parse(fs.readFileSync(path.join(__dirname, 'build-env.json')));
    for (const key in envConfig) {
      process.env[key] = envConfig[key];
    }
    console.log('[DEBUG] Production mode: Loaded environment variables from build-env.json.');
  } catch (error) {
    console.error('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
    console.error('FATAL ERROR: Could not load build-env.json in packaged app!');
    console.error('The app will not work without this configuration file.');
    console.error(error);
    console.error('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
    // In a real scenario, you'd want to show an error dialog and quit.
    // dialog.showErrorBox('Fatal Error', 'Configuration is missing.');
    // app.quit();
  }
} else {
  // In development, load variables from the .env file
  require('dotenv').config();
  console.log('[DEBUG] Development mode: Loaded environment variables from .env file.');
}

// Debug environment variables
console.log('[DEBUG] NODE_ENV:', process.env.NODE_ENV);
console.log('[DEBUG] GATEWAY_URL:', process.env.GATEWAY_URL);
console.log('[DEBUG] DISCOVERY_URL:', process.env.DISCOVERY_URL);
console.log('[DEBUG] BACKEND_URL:', process.env.BACKEND_URL);
console.log('[DEBUG] OAUTH_REDIRECT_URI:', process.env.OAUTH_REDIRECT_URI);

// Always load from static files - no development server dependency
...
const SERVER_URI_DEFAULT = process.env.NODE_ENV === 'production'
  ? process.env.GATEWAY_URL || "wss://ws.nova-voice.com"
  : "ws://localhost:5026";
const DISCOVERY_SERVICE_URL = process.env.NODE_ENV === 'production'
  ? process.env.DISCOVERY_URL || "https://api.nova-voice.com"
  : "http://localhost:5025";
...
const BACKEND_URL = process.env.NODE_ENV === 'production'
  ? process.env.BACKEND_URL || 'https://api.nova-voice.com'
  : 'http://localhost:8080'; // Local backend URL for auth

// OAuth redirect URI
const OAUTH_REDIRECT_URI = process.env.NODE_ENV === 'production'
  ? process.env.OAUTH_REDIRECT_URI || 'https://auth.nova-voice.com/auth/callback'
  : 'http://localhost:8080/auth/callback';

// Authentication storage
const AUTH_STORAGE_PATH = path.join(os.homedir(), '.nova-voice', 'auth.json');
...
function saveAuthData(user, token) {
  try {
    ...
    fs.writeFileSync(AUTH_STORAGE_PATH, JSON.stringify(authData, null, 2));
...
function loadAuthData() {
  try {
    if (fs.existsSync(AUTH_STORAGE_PATH)) {
      const authData = JSON.parse(fs.readFileSync(AUTH_STORAGE_PATH, 'utf8'));
      ...
      if (tokenAge < maxAge) {
        console.log('Authentication data loaded from storage');
        return authData;
      } else {
        console.log('Authentication data expired, removing');
        clearAuthData();
      }
    }
  } catch (error) {
...
function createWindow() {
  ...
  mainWindow.on('closed', () => {
    console.log('[MAIN WINDOW] Main window closed, isLoggingOut =', isLoggingOut);
    // Don't quit the app if we're logging out (we want to show login window instead)
    // Also don't quit if there's a login window (logout scenario)
    if (!isLoggingOut && !loginWindow) {
      console.log('[MAIN WINDOW] Quitting app due to main window close');
      app.quit();
    } else {
      console.log('[MAIN WINDOW] Not quitting app because isLoggingOut = true or login window exists');
    }
  });
...
function createLoginWindow() {
  ...
  loginWindow = new BrowserWindow({
    width: 628,
    height: 412,
    x: Math.floor((screenWidth - 628) / 2),
    y: Math.floor((screenHeight - 412) / 3),
    transparent: true,
    backgroundColor: '#00000000',
    frame: false,
    alwaysOnTop: true,      // Always stay on top like main window
    resizable: false,
    skipTaskbar: true,      // Hide from taskbar and Alt+Tab
    show: false,
    webPreferences: {
  ...
  // Load local login.html file instead of directly going to Google OAuth
  const loginHtmlPath = path.join(__dirname, 'login.html');
  loginWindow.loadFile(loginHtmlPath);
  loginWindow.once('ready-to-show', () => loginWindow && loginWindow.show());

  // Ensure login window always stays on top
  loginWindow.on('blur', () => {
    // Re-focus the window to keep it on top
    if (loginWindow && !loginWindow.isDestroyed()) {
      setTimeout(() => {
        if (loginWindow && !loginWindow.isDestroyed()) {
          loginWindow.focus();
...
// Client functionality - WebSocket connection
function connectToGateway(token) {
  return new Promise(async (resolve, reject) => {
    ...
    console.log('[DEBUG] Connecting to gateway...');
    console.log('[DEBUG] SERVER_URI_DEFAULT:', SERVER_URI_DEFAULT);
    console.log('[DEBUG] DISCOVERY_SERVICE_URL:', DISCOVERY_SERVICE_URL);

    try {
      // Try to discover gateway first
      const baseGatewayUri = await discoverGateway();
      console.log('[DEBUG] Discovered gateway URI:', baseGatewayUri);

      // Add token to the WebSocket URL if provided
      const gatewayUri = token ? `${baseGatewayUri}?token=${token}` : baseGatewayUri;

      ws = new WebSocket(gatewayUri);
      ...
      ws.on('error', (error) => {
        console.error('[DEBUG] WebSocket connection failed:', error.message);
        console.error('[DEBUG] Full error:', error);
        console.log('[DEBUG] Make sure the gateway service is running on port 5026');
        console.log('[DEBUG] Final gateway URI attempted:', gatewayUri);
        isConnected = false;
        try { if (pingInterval) { clearInterval(pingInterval); pingInterval = null; } } catch (_) {}
        if (mainWindow) {
          mainWindow.webContents.send('connection-status', { connected: false });
        }
        reject(new Error(`Cannot connect to gateway: ${error.message}`));
      });
      ...
      setTimeout(() => {
        if (!isConnected) {
          ...
          console.log('Make sure the gateway service is running and accessible');
          reject(new Error('Connection timeout - gateway may not be running'));
        }
      }, 5000);

    } catch (error) {
      console.error('Failed to discover gateway:', error.message);
      console.log('Trying to connect to default gateway...');
      if (mainWindow) {
        mainWindow.webContents.send('connection-status', { connected: false });
      }
...
// IPC handlers for opening OAuth in external browser
...
ipcMain.handle('open-github-auth', async () => {
  try {
    ...
    console.error('Error opening GitHub auth URL:', error);
    return { success: false, error: error.message };
  }
});

ipcMain.handle('open-microsoft-auth', async () => {
  try {
    ...
    const authUrl = `https://login.microsoftonline.com/${MICROSOFT_TENANT_ID}/oauth2/v2.0/authorize?` +
      ...
      `response_mode=query&` +
      `state=provider%3Dmicrosoft`;

    console.log('Opening Microsoft OAuth URL:', authUrl);
    await shell.openExternal(authUrl);
    startAuthPolling();

    return { success: true };
  } catch (error) {
    console.error('Error opening Microsoft auth URL:', error);
    return { success: false, error: error.message };
  }
});
...
ipcMain.handle('open-discord-auth', async () => {
  try {
    if (!DISCORD_CLIENT_ID || !DISCORD_CLIENT_SECRET) {
      return { success: false, error: 'Discord OAuth credentials not configured' };
    }
    ...
    console.error('Error opening Discord auth URL:', error);
    return { success: false, error: error.message };
  }
});

// IPC handlers for client functionality
ipcMain.handle('connect-gateway', async (event, token) => {
  console.log('[DEBUG] connect-gateway IPC called');
  console.log('[DEBUG] Token provided:', token ? 'YES (length: ' + token.length + ')' : 'NO');
  try {
    await connectToGateway(token);
    console.log('[DEBUG] Gateway connection successful');
    return { success: true };
  } catch (error) {
    console.log('[DEBUG] Gateway connection failed:', error.message);
    return { success: false, error: error.message };
  }
});

// Simple auth IPC: replace with real backend as needed

ipcMain.handle('auth-login', async (_event, { email, password }) => {
  try {
    // Basic placeholder authentication: require both fields
    if (typeof email === 'string' && email.includes('@') && typeof password === 'string' && password.length >= 4) {
      ...
      // Optional: create subtitle overlay lazily or now
...
ipcMain.handle('auth-callback', async (event, authCode) => {
  try {
    ...
    saveAuthData(session.user, session.token);
...
ipcMain.handle('auth-logout', async () => {
  try {
    ...
    // Set logout flag to prevent app.quit() when windows close
    isLoggingOut = true;
    lastLogoutTime = Date.now();
    console.log('[LOGOUT] Set isLoggingOut = true');

    // Stop any ongoing auth polling to prevent auto-login
    stopAuthPolling();
    console.log('[LOGOUT] Stopped auth polling');

    // Clear browser session data to prevent cached OAuth state
    try {
      await session.defaultSession.clearStorageData({
        storages: ['cookies', 'localstorage', 'sessionstorage']
      });
      console.log('[LOGOUT] Cleared browser session data');
    } catch (error) {
      console.log('[LOGOUT] Error clearing session data:', error.message);
    }

    // Logout from backend FIRST to invalidate server session
    const tokenToLogout = authToken; // Save token before clearing
    if (tokenToLogout) {
      try {
        console.log('[LOGOUT] Logging out from backend...');
        console.log('[LOGOUT] Backend URL:', BACKEND_URL);
        console.log('[LOGOUT] Token length:', tokenToLogout.length);
        ...
        console.log('[LOGOUT] Backend logout response status:', logoutResponse.status);
        if (logoutResponse.ok) {
          const responseData = await logoutResponse.json();
          console.log('[LOGOUT] Backend logout successful:', responseData);
        } else {
          const errorText = await logoutResponse.text();
          console.log('[LOGOUT] Backend logout failed, status:', logoutResponse.status, 'response:', errorText);
        }
      } catch (error) {
        console.log('[LOGOUT] Error logging out from backend:', error.message);
      }
    } else {
      console.log('[LOGOUT] No auth token to logout from backend');
    }

    // Now clear authentication state after backend logout
    isAuthenticated = false;
    currentUser = null;
    authToken = null;
    console.log('[LOGOUT] Cleared authentication state');

    // Clear authentication data from disk
    clearAuthData();
    console.log('[LOGOUT] Cleared authentication data from disk');

    // Disconnect from gateway and stop modules
    try { disconnectFromGateway(); } catch (_) {}
    try { if (typingSimulationManager) typingSimulationManager.stop(); } catch (_) {}
    try { if (liveTranscriptionManager) liveTranscriptionManager.stop(); } catch (_) {}
    console.log('[LOGOUT] Stopped modules');

    // Create login window first to ensure there's always a window open
    // Close any existing login window first to ensure clean state
    if (loginWindow && !loginWindow.isDestroyed()) {
      loginWindow.close();
      loginWindow = null;
    }
    ...
    createLoginWindow();
    console.log('[LOGOUT] Login window created');
    ...
    console.log('[LOGOUT] Closed subtitle window');

    // Close main window
    if (mainWindow && !mainWindow.isDestroyed()) {
      console.log('[LOGOUT] Closing main window...');

      // Set up a one-time listener for when the main window actually closes
      mainWindow.once('closed', () => {
        console.log('[LOGOUT] Main window actually closed');
        mainWindow = null;

        // Reset logout flag only after window has closed
        isLoggingOut = false;
        console.log('[LOGOUT] Reset isLoggingOut = false');
      ...
      // Main window was already closed or doesn't exist
      console.log('[LOGOUT] Main window already closed or non-existent');
...
// Discover gateway using discovery service
async function discoverGateway() {
  try {
    const url = `${DISCOVERY_SERVICE_URL}/discovery/least-loaded`;
    console.log('[DEBUG] Discovery URL:', url);
    ...
    return new Promise((resolve, reject) => {
      const req = protocol.get(url, { timeout: 5000 }, (res) => {
        ...
        res.on('end', () => {
          console.log('[DEBUG] Discovery response status:', res.statusCode);
          console.log('[DEBUG] Discovery response headers:', res.headers);
          console.log('[DEBUG] Discovery response data (first 500 chars):', data.substring(0, 500));

          try {
            const response = JSON.parse(data);
            console.log('[DEBUG] Discovery response parsed:', response);
            if (response.success && response.gateway) {
              const gateway = response.gateway;
              // Use the full WebSocket URL from discovery response, not localhost
              const gatewayUri = gateway.ws_url || `ws://localhost:${gateway.port}`;
              console.log(`[DEBUG] Discovered gateway: ${gateway.gateway_id} (port ${gateway.port})`);
              console.log('[DEBUG] Gateway URI:', gatewayUri);
              resolve(gatewayUri);
            } else {
              console.log('[DEBUG] Discovery failed, using default gateway');
              console.log('[DEBUG] Default SERVER_URI_DEFAULT:', SERVER_URI_DEFAULT);
              resolve(SERVER_URI_DEFAULT);
            }
          } catch (e) {
            console.log('[DEBUG] Error parsing discovery response:', e);
            console.log('[DEBUG] Raw response data:', data);
            console.log('[DEBUG] Using default gateway:', SERVER_URI_DEFAULT);
            resolve(SERVER_URI_DEFAULT);
          }
        });
      });
      ...
      req.on('error', (error) => {
        console.log('Discovery service error, using default gateway:', error.message);
        console.log('Make sure the discovery service is running on port 5025');
        resolve(SERVER_URI_DEFAULT);
      });

      req.on('timeout', () => {
        console.log('Discovery service timeout, using default gateway');
        req.destroy();
        resolve(SERVER_URI_DEFAULT);
      });
    });
  } catch (error) {
    console.log('Discovery failed, using default gateway:', error.message);
    return SERVER_URI_DEFAULT;
  }
}

// Polling mechanism for authentication status
...
function startAuthPolling() {
  ...
  authPollingInterval = setInterval(async () => {
    try {
      // Skip polling if we recently logged out (prevent auto-login race condition)
      const timeSinceLogout = Date.now() - lastLogoutTime;
      if (timeSinceLogout < 2000) { // 2 second cooldown after logout
        return;
      }

      // Check if authentication was successful
      console.log('[POLLING] Checking auth status at:', `${BACKEND_URL}/auth/status`);
      const authResponse = await fetch(`${BACKEND_URL}/auth/status`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' }
      });

      console.log('[POLLING] Response status:', authResponse.status);
      if (authResponse.ok) {
        const authData = await authResponse.json();
        console.log('[POLLING] Auth data received:', { authenticated: authData.authenticated, hasUser: !!authData.user });
        if (authData.authenticated && authData.user) {
          console.log('[DEBUG] Authentication detected via polling:', authData);
          ...
          // Send auth success to the main window
          if (mainWindow && !mainWindow.isDestroyed()) {
            console.log('[DEBUG] Sending auth-success to main window');

            // Wait for the content to be ready before sending auth event
            const sendAuthSuccess = () => {
              if (mainWindow && !mainWindow.isDestroyed() && mainWindow.webContents && !mainWindow.webContents.isLoading()) {
                console.log('[DEBUG] Content is ready, checking if electronAPI is available...');

                // Test if electronAPI is available by executing a small script
                mainWindow.webContents.executeJavaScript('typeof window.electronAPI !== "undefined"')
                  .then((isAPIReady) => {
                    if (isAPIReady) {
                      console.log('[DEBUG] electronAPI is available, sending auth-success event');
                      mainWindow.webContents.send('auth-success', {
                        token: authData.token,
                        user: authData.user
                      });
                    } else {
                      console.log('[DEBUG] electronAPI not ready yet, retrying in 500ms...');
                      setTimeout(sendAuthSuccess, 500);
                    }
                  })
                  .catch((error) => {
                    console.log('[DEBUG] Error checking electronAPI availability:', error.message);
                    console.log('[DEBUG] Retrying in 500ms...');
                    setTimeout(sendAuthSuccess, 500);
                  });
              } else {
                console.log('[DEBUG] Content not ready yet, retrying in 500ms...');
                setTimeout(sendAuthSuccess, 500);
              }
            };
            ...
            // Close login window
            if (loginWindow && !loginWindow.isDestroyed()) {
              console.log('[DEBUG] Closing login window');
              loginWindow.close();
            }
            ...
            console.log('[DEBUG] Main window not available to send auth-success');
          }

          // Stop polling
          console.log('[DEBUG] Stopping auth polling');
          stopAuthPolling();
        }
      }
    } catch (e) {
...
// Register custom protocol for OAuth callback (fallback)
app.setAsDefaultProtocolClient('nova-auth');
...
app.on('open-url', (event, url) => {
  ...
  try {
    ...
    if (urlObj.protocol === 'nova-auth:' && urlObj.pathname === '/callback') {
      ...
      if (token && userId && email && name) {
        console.log('Received auth via custom protocol');
...
app.whenReady().then(async () => {
  ...
  if (authData) {
    console.log('Found existing authentication, validating token...');

    // Validate token with backend
    const isValid = await validateToken(authData.token);
    if (isValid) {
      console.log('Token is valid, proceeding with authenticated session');
      isAuthenticated = true;
      currentUser = authData.user;
      authToken = authData.token;
      ...
      // Send auth-success event to frontend so it can connect to gateway
      setTimeout(() => {
        if (mainWindow && !mainWindow.isDestroyed()) {
          console.log('[DEBUG] Sending existing auth-success to main window');
          mainWindow.webContents.send('auth-success', {
            token: authData.token,
            user: authData.user
          });
      ...
      console.log('Token is invalid, clearing auth data and showing login');
      clearAuthData();
      createLoginWindow();
    }
  } else {
    console.log('No existing authentication found, showing login');
    createLoginWindow();
  }
});
...
app.on('window-all-closed', () => {
  console.log('[APP] window-all-closed event fired, isLoggingOut =', isLoggingOut);
  console.log('[APP] Current windows:', BrowserWindow.getAllWindows().length);
  // Don't quit if we're logging out (login window will be created)
  // Or if there's still a login window
  if (process.platform !== 'darwin' && !isLoggingOut && !loginWindow) {
    console.log('[APP] Quitting app due to all windows closed');
    app.quit();
  } else {
    console.log('[APP] Not quitting app because isLoggingOut = true, login window exists, or on macOS');
  }
});
